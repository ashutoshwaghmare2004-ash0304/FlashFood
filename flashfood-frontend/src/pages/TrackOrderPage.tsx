...
const TrackOrderPage = () => {
  ...
  {new Date(`1970-01-01T${step.time}`).toLocaleTimeString([], {
    hour: "numeric",
    minute: "2-digit",
    hour12: true,
  })}
          ...
          {order.status !== 'delivered' && (
            <div className="mt-6 p-4 bg-blue-50 rounded-lg">
              <p className="text-sm font-semibold text-blue-900">
                Estimated Delivery: {`${new Date(order.estimatedDelivery).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })} , ${new Date(order.estimatedDelivery).toLocaleDateString('en-US')}`}
